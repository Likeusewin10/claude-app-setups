...
如果 `raw.githubusercontent.com` 无法访问（国内网络常见），使用唯一认可的备用源 jsDelivr（回源 GitHub 的公共 CDN，已固定提交号 `3632cc9` 以避免 CDN 缓存导致新旧混版）。把上面 6 个路径中的前缀替换为：

```text
https://cdn.jsdelivr.net/gh/Likeusewin10/claude-app-setups@3632cc9/apps/codex-media-skills/skills/
```
...
```text
nebula-image-gen/SKILL.md                 fec7470654d8ef35e0f7c5e2583fbc7f60412450154c194846e868a0a7cf0982
nebula-image-gen/agents/openai.yaml       3eb3770b5c4569b51b676bef4645f0ee9eee15ebcd3e24ff4d16bac462bcf5e7
nebula-image-gen/references/api.md         1d39dff9022b0606b4d481e6565f68f907cf5abd1dd7dfd800c0e0f22de6d81f
...
4. 告诉用户从下一轮对话开始，用**点名 skill** 的说法来触发，例如“使用 apinebula 生图：一张……”“用 nebula-image-gen 改图：把……”“使用 seedance 生视频：……”；也可以在输入框键入 `$` 从列表中选择 skill（`$nebula-image-gen` / `$ark-video-gen`）后接上需求。提醒用户不要只说“生成一张图”“生成一个视频”——泛化说法可能不会命中 skill，而是落到平台内置的预览能力，拿不到本地文件。
